...
int main(){
    ...
    std::vector<float> input(num * channels * width * height);
    std::vector<float> output(num * 1000);
    ...
    while (true)
    {
        ...
        cudaMemcpy(device_buffers[0], input.data(), num * channels * height * width * sizeof(float), cudaMemcpyHostToDevice);
        exec_context->executeV2(device_buffers);
        cudaMemcpy(output.data(), device_buffers[1], num * 1000 * sizeof(float), cudaMemcpyDeviceToHost);

        end = std::chrono::high_resolution_clock::now();
        std::cout << "前向时间：" << std::chrono::duration<double, std::milli>(end - start).count() << std::endl;
        start = end;
        int idx = std::max_element(output.begin(), output.end()) - output.begin();
        end = std::chrono::high_resolution_clock::now();
